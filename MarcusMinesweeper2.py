    ...
    def gameBoard(self, gridSize, root, frame):
        ...
        for i in range(1,gridSize+1):
            ...
            for j in range(1,gridSize+1):       
                col.append(Cell(0, 0, 0, j-1, i-1, 0, root, frame))
            board.append(col)
        return board 
        ...
        while(zeroList): 
            for j in moves:
                try:
                    ...
                    if (cordNewX > -1 and cordNewY > -1) and (cordNewX < minesweeper.gridSize and cordNewY < minesweeper.gridSize):#finns koordinater inom spelmatrisen
                        if(self.board[cordNewY][cordNewX].clickStatus == 0):                                        
                            if(self.board[cordNewY][cordNewX].minesAround != 0):
                                ...
                                if(self.board[cordNewY][cordNewX].flagStatus == 0):
                                    ...
                                    self.board[cordNewY][cordNewX].button["text"] = "" 
                                    self.board[cordNewY][cordNewX].clickStatus = 1                    
                except IndexError:
                    pass               
            if(self.board[y][x].flagStatus == 0):
...
class Cell():
    def __init__(self, mineStatus, clickStatus, minesAround, cordX, cordY, flagStatus, root, frame):
        ...
        self.button = Button( #knapp som läggs i spelmatrisen
            frame,
            command=lambda: Minesweeper.click(minesweeper,self.cordX, self.cordY), text="*") 
...
def gameMode(optPop, gridSize, mineCount):
    # global minesweeper
    minesweeper = Minesweeper(gridSize, mineCount)
