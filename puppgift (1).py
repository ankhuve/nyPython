...
def ritaSpelplan(listor, riktn, langd, counter):
    for i in range(langd):
        ...
        if riktn == "H":
            del spelplan[rader.index(k[1:])][kolumner.index(k[0])+i] # Radera n0llan
            spelplan[rader.index(k[1:])].insert(kolumner.index(k[0])+i, "#") # Lägg till '#' på n0llans plats
        if riktn == "V":
            del spelplan[rader.index(k[1:])+i][kolumner.index(k[0])] # Radera n0llan
...
def gissning(listor):
    ...
    if skott in oppTagnaRutor:
        print("Träff! ")
        del oppTagnaRutor[tagnaRutor.index(skott)]
